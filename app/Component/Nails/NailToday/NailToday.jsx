...
class NailToday extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      onFrame: false,
  ...
  _render_nails_nouns(){
    /*
    in order to fit the requirement of common used renderNodesRows(), reduce the modification to existed Nails,
    we create a props obj, include all the props the renderNodesRows needed,
    and a nounsList 'exclude' the corner of the day.
    */
    let cornerList = this.props.unitBasic.nounsList.filter((node, index)=>{
      return node != this.props.todayNode
    });
    let nounsProps = {
      unitBasic: {nounsList: cornerList},
      unitId: this.props.unitId,
      nounsBasic: this.props.nounsBasic
    }
    let nodesDOM = renderNodesRows(nounsProps, styles);

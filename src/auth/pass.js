...
pass.use(function(req, res, next) {
  ...
  if (
    ...
    jwt.verify(token, keyUsed, function(err, decoded) {
      if (err) {
        // though the token was invalid, we still allow some of the api pass
        // check like the way if no token
        ...
        _check_pathAllowed(req, res, next);
      } else {
...
const _check_pathAllowed = (req, res, next) => {

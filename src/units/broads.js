...
function _handle_units_broad_POST(req, res){
  jwt.verify(req.headers['token'], verify_key, function(err, payload) {
    if (err) {
      ...
      _select_withPromise_BasicNoLength(BROADS_USERS_UNITS, mysqlForm.accordancesList).then((results)=>{
        let sendingData={temp: {}},
            unitId = req.reqUnitId;
        let permit = results.every((row)=>{
              return row.id_unit !== unitId
            });
        if(!permit) throw {status: 400, err: "Already broadcasted!"};
        return _insert_withPromise_Basic("broads", {id_user: userId, id_unit: unitId}).then(()=>{
          return sendingData
        })
      }).then((sendingData)=>{
        _res_success(res, sendingData, "post req: unit broad, complete.");
      }).catch((errObj)=>{
        console.log("error occured during getting unit broad promise: "+errObj.err)
...
execute.post('/', function(req, res){
  ...
  _handle_units_broad_POST(req, res);
})
